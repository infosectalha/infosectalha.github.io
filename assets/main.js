/* assets/main.js
   Handles:
   - mobile sidebar toggle (left slide)
   - animated background: lightweight canvas (binary rain + nodes/lines)
...
// Animated background (canvas) - binary rain + network nodes (optimized)
(function(){
  ...
  // nodes
  const nodes = [];
  const NODE_COUNT = Math.min(60, Math.floor((w*h)/160000));
  ...
  function draw() {
    ...
    for(let i=0;i<nodes.length;i++){
      const n = nodes[i];
      const pulse = 0.6 + Math.sin(Date.now()*0.003 + n.phase)*0.6;
    ...
    // binary rain (light, column-based)
    ...
    for(let c=0;c<cols;c++){
      ...
      colY[c] += 1;
      if(colY[c]*16 > h && Math.random() > 0.975) colY[c] = 0;
    }
